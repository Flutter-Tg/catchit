...
Future<DataState<DetailEntity>> serializTiktok5(
  ...
  if (response.statusCode == 200) {
    ...
    if (json.containsKey('video')) {
      for (var video in json['video']) {
        FileInfoParam? info = await getFileInfo.fileInfo(video);
        videos.add(
          DetailFile(
            url: video,
            name: 'tiktok-video-${generateRandomString(16)}',
            type: info == null ? 'mp4' : info.type,
            size: info?.size,
          ),
        );
      }
    ...
    if (json.containsKey('description') &&
        json['description'][0].trim().isNotEmpty) {
    ...
    String? avatarThumb;
    if (json.containsKey('avatar_thumb')) {
      avatarThumb = await getImageByt(json['avatar_thumb'][0]);
    }
